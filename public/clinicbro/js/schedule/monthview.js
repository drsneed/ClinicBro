  ...
  renderDays() {
    ...
    for (let week = 0;week < 6; week++) {
      ...
      for (let day = 0;day < 7; day++) {
        ...
        days.push(x`<td id="${id}" class="${dayClass}"><span class="num ${numClass}">${num}</span>
          ${this.appointments.filter((appt) => sameDay(appt.start, thisDaysDate)).map((appt) => x`<x-appt name="${appt.name}" start="${appt.start}" end="${appt.end}"></x-appt>`)}</td>`);
        i4++;
      }
