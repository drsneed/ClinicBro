...
var header_storage: [4 * 1024]u8 = undefined;

fn fetchAlloc(allocator: std.mem.Allocator, url: []const u8) !std.ArrayList(u8) {
    var client = std.http.Client{ .allocator = allocator };
    defer client.deinit();
    @memset(&header_storage, 0);
    var responseData = std.ArrayList(u8).init(allocator);
    errdefer responseData.deinit();
    const fetchOptions = std.http.Client.FetchOptions{ 
        .server_header_buffer = &header_storage,
        .method = .GET, // some servers don't respond to .HEAD even though that's all we are after
        .location = .{ .url = url }, 
        .response_storage = .{ .dynamic = &responseData }
    };
    const fetchResult = try client.fetch(fetchOptions);
   
    if (fetchResult.status != std.http.Status.ok) {
...
pub const MapViewer = struct {
    ...
    pendingRequests: std.AutoHashMap(TilePosition, *jetzig.Request),
    pendingSearchRequest: ?*jetzig.Request = null,
    centerX: f32 = 0,
    centerY: f32 = 0,
    camZoom: u5 = 4,
    ...
    const Tile = struct { data: []u8 };

    pub fn init(allocator: std.mem.Allocator) MapViewer {
        var viewer = MapViewer.init_events(MapViewer{
            .tileCache = std.AutoHashMap(TilePosition, Tile).init(allocator),
            .pendingRequests = std.AutoHashMap(TilePosition, *jetzig.Request).init(allocator),
            .allocator = allocator,
        });
        viewer.centerTo(2.3200, 48.8589);
        viewer.setName("map-viewer");
