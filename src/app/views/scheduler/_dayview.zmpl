...
<day-view id="schedule" current_date="{{current_date}}">
    @zig {
        for(appointments.array.array.items) |appt| {
            ...
            const slot_name = try std.fmt.bufPrint(&slot_buffer, "{s}:{s}", .{appt_from[0..2], if(appt_from[2]>'2') "30" else "00"});
            <dv-appt slot="{{slot_name}}" appt_id="{{appt_id}}" appt_title="{{title}}"
                ...
                hx-swap="outerHTML" hx-trigger="dblclick target:dv-appt"></dv-appt>
        }
    }
</day-view>